...
def apply_ta(data, short_window, long_window):
    ...
    data['ema_long'] = ta.trend.EMAIndicator(data['close'], window=long_window).ema_indicator()

    return data

def buy_trigger(t, data):
    return data.loc[t, 'ema_short'] > data.loc[t, 'ema_long']
...
def evaluate_fitness(individual):
    data = fetch_data()
    data = apply_ta(data, individual[0], individual[1]) #SMA
    final_capital = evaluate_bot(data, individual[0], individual[1]) #SMA
