...
def apply_ta(data, short_window, long_window):
    stoch = ta.momentum.StochasticOscillator(data['high'], data['low'], data['close'], window=short_window, smooth_window=long_window)
    data['%K'] = stoch.stoch()
...
def buy_trigger(t, data):
    return data.loc[t, '%K'] > data.loc[t, '%D']
...
toolbox.register("attr_int", random.randint, 5, 20)
...
toolbox.register("mutate", tools.mutUniformInt, low=5, up=20, indpb=MUTATION_RATE)
